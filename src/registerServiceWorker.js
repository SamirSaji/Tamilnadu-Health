...
export default function register() {
  ...
  // console.info('production',process.env.NODE_ENV);
...
function registerValidSW(swUrl) {
  navigator.serviceWorker
    .register(swUrl)
    .then(registration => {
      // console.info('REGISTRATIONSW',registration);
      if('periodicSync' in registration){
        console.info('Registering Popluations')
        registration.periodicSync.register('population-sync',{
          minInterval: 2 * 60 * 1000//24 * 60 * 60 * 1000
        });
      }
      registration.onupdatefound = () => {
        sendNotification('App installed for offline use!');
        const installingWorker = registration.installing;
        installingWorker.onstatechange = () => {
          if (installingWorker.state === 'installed') {
            if (navigator.serviceWorker.controller) {
              sendNotification('Reloading to reflect updated content')
